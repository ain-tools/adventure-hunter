...
@Component({
  selector: 'app-level-one-v',
  templateUrl: './level-one-v.component.html',
  styleUrls: ['./level-one-v.component.css']
})
export class LevelOneVComponent implements OnInit {
    ...
    if(characterClass === 'druid'){
      ...
      setTimeout(()=>{
        ...
        if(document.getElementsByClassName('enemyName')[0]){
          ...
          enemyPortrait.setAttribute('src','./assets/monster12.png');
        }
  ...
  checkIfDefeated(){
    if(this.arrayOfClasses.length == 0){
      if(document.getElementsByClassName('levelOneF')){
        this.messageEvent.emit(this.levelOneCompletedV);
