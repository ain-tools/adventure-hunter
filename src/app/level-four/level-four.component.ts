...
export class LevelFourComponent implements OnInit {
    ...
    if(characterClass === 'druid'){
      ...
      setTimeout(()=>{
        ...
        if(document.getElementsByClassName('enemyName')[0]){
          enemyName.innerHTML = "Legendary Corrupted Mage";
          let enemyDescription = document.getElementsByClassName('enemyDescription')[0] as HTMLElement;
          enemyDescription.innerHTML = "She's very beautiful, nevertheless, you must kill her. An insignia shows she's from the same guild as the corrupted warrior from before. She looks corrupted as well.";
          let enemyPortrait = document.getElementsByClassName('enemyPortrait')[0] as HTMLElement;
          enemyPortrait.setAttribute('src','./assets/monster7.png');
        }
      },100)
      ...
      setTimeout(()=>{
        let enemyName = document.getElementsByClassName('enemyName')[0] as HTMLElement;
        enemyName.innerHTML = "Lightning Bolt";
        let enemyDescription = document.getElementsByClassName('enemyDescription')[0] as HTMLElement;
        enemyDescription.innerHTML = "A bolt of lightning casted by the mage. Pushes back ennemies.";
        let enemyPortrait = document.getElementsByClassName('enemyPortrait')[0] as HTMLElement;
        enemyPortrait.setAttribute('src','./assets/electricBlast.png');
      },100)
 ...
 revealed:boolean=false;
  ...
  //monster1
  fullMonster1Life : number = 15;
  monster1Life : number = 15;
  //monster2
  fullMonster2Life : number = 50;
  monster2Life : number = 8;
  ...
  checkIfDefeated(){
    if(this.arrayOfClasses.length == 0){
      if(document.getElementsByClassName('levelOneF')){
        this.messageEvent.emit(this.levelFourCompletedF);
...
victoryDungeonAlert = false;
  ...
  if(monster2Select){
    ...
    if(document.getElementsByClassName(this.monsterCoor1)[0] && document.getElementsByClassName(this.monsterCoor1)[0].hasChildNodes() && checkEnemyOrAlly1.className.slice(12,18) == "target" ){
      ...
      let node = document.createElement("B");                 // Create a <b> node
      let textnode = document.createTextNode("110 DMG!");         // Create a text node
      let willAppendToTarget = node.appendChild(textnode);
      DMGwarning.style.color = "red";      
      DMGwarning.appendChild(willAppendToTarget);  
      let spiderSound = new Audio("./assets/casterBlastEffect.mp3");
              spiderSound.play();       
      setTimeout(()=>{
      ...
      let maxOutHP = this[monsterTarget];
        let maxOutHP2 = maxOutHP -= 110;
          if(maxOutHP2 < 0 ){
            this[monsterTarget] = 0;
            monsterTarget1.parentElement.removeChild(monsterTarget1);
            monster2.parentElement.removeChild(monster2);
            if(!document.getElementsByClassName('4_2')[0].hasChildNodes()){
              document.getElementsByClassName('4_2')[0].appendChild(monster2);
            }
          }else{
            this[monsterTarget] -= 110;
            monster2.parentElement.removeChild(monster2);
            if(!document.getElementsByClassName('4_2')[0].hasChildNodes()){
              document.getElementsByClassName('4_2')[0].appendChild(monster2);
            }
